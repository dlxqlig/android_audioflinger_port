...
static char* sample_buffer[BUFFER_SIZE];
status_t Sample::doLoad()
{
    uint32_t sampleRate;
    int numChannels;
    audio_format_t format;
    MemoryDealer* dealer = new MemoryDealer(BUFFER_SIZE);
    sp<IMemory> p= dealer->allocate(BUFFER_SIZE);
    ALOGV("Sample::doLoad start");
    ALOGV("Start decode, url:%s", mUrl);
    if (mUrl) {
        //p = MediaPlayer::decode(mUrl, &sampleRate, &numChannels, &format);
    	ALOGV("start to read data");
    	int fd = open(mUrl, O_RDONLY);
    	::read(fd, sample_buffer, BUFFER_SIZE);
    	memcpy(p->pointer(), sample_buffer, p->size());
    	close(fd);
    } else {
    	::read(mFd, sample_buffer, BUFFER_SIZE);
        ...
        float rightVolume, int priority, int loop, float rate)
{
    ...
    { // scope for the lock
        ...
        uint32_t sampleRate = 44100;//uint32_t(float(sample->sampleRate()) * rate + 0.5);
        ...
        newTrack = new AudioTrack(streamType, sampleRate, sample->format(),
                channels, sample->getIMemory(), AUDIO_OUTPUT_FLAG_DIRECT, callback, userData);
#else
        newTrack = new AudioTrack(streamType, sampleRate, sample->format(),
...
{
	ALOGV("@@@@@@@@@@@SoundChannel::callback called");
...
//    ALOGV("SoundChannel::process event %d", event);
    ...
    if (event == AudioTrack::EVENT_MORE_DATA) {
    	ALOGV("@@@@@@@@@@@SoundChannel::process EVENT_MORE_DATA mPos %d, mAudioBufferSize %d", mPos, mAudioBufferSize);
