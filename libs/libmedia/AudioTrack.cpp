        ...
        audio_io_handle_t output)
{
    ...
    if (flags & AUDIO_OUTPUT_FLAG_FAST) {
        ...
        if (mAudioTrackThread != 0) {
            tid = androidGetTid();//mAudioTrackThread->androidGetTid();
...
bool AudioTrack::processAudioBuffer(const sp<AudioTrackThread>& thread)
{
    ...
    bool active = mActive;
    mLock.unlock();
    ALOGV("processAudioBuffer loop framesAvailable %d", mFrameCount);
    // since mLock is unlocked the IAudioTrack and shared memory may be re-created,
    // so all cblk references might still refer to old shared memory, but that should be benign
    ALOGV(" #####processAudioBuffer user: %x, server: %x, flags %04x", cblk->user, cblk->server, cblk->flags);
    ...
    ALOGV("processAudioBuffer loop frames %d", frames);
    do {
        ...
        ALOGV("processAudioBuffer loop 3");
        if (mFormat == AUDIO_FORMAT_PCM_8_BIT && !(mFlags & AUDIO_OUTPUT_FLAG_DIRECT)) {
        ...
        ALOGV("processAudioBuffer loop 4 frames %d", frames);
        releaseBuffer(&audioBuffer);
    }
    while (frames);
