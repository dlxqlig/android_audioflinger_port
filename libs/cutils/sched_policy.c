...
static int add_tid_to_cgroup(int tid, SchedPolicy policy)
{
    ...
    if (fd < 0) {
        ALOGE("add_tid_to_cgroup failed; policy=%d\n", policy);
        return -1;
    }
    ...
    *ptr = '\0';
    ...
    if (write(fd, ptr, end - ptr) < 0) {
        ...
        if (errno == ESRCH)
                return 0;
        ALOGW("add_tid_to_cgroup failed to write '%s' (%s); policy=%d\n",
...
static int getSchedulerGroup(int tid, char* buf, size_t bufLen)
{
    ...
    ALOGE("Failed to find cpu subsys");
    fclose(fp);
    return -1;
 out_bad_data:
...
int set_sched_policy(int tid, SchedPolicy policy)
{
    ...
    switch (policy) {
    case SP_BACKGROUND:
        ALOGD("vvv tid %d (%s)", tid, thread_name);
        break;
    case SP_FOREGROUND:
    case SP_AUDIO_APP:
    case SP_AUDIO_SYS:
    	ALOGD("^^^ tid %d (%s)", tid, thread_name);
        break;
    case SP_SYSTEM:
    	ALOGD("/// tid %d (%s)", tid, thread_name);
        break;
    ...
    if (__sys_supports_schedgroups) {
    	...
    	printf("sched_setscheduler is called\n");
