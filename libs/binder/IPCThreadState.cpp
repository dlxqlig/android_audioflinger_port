...
#define IF_LOG_TRANSACTIONS() ALOGD("transact"); if (true)
#define IF_LOG_COMMANDS() ALOGD("ipc"); if (true)
#define LOG_REMOTEREFS(...) ALOGD("remoterefs", __VA_ARGS__) ; if (true)
#define IF_LOG_REMOTEREFS() ALOGE("remoterefs"); if (true)
#define LOG_THREADPOOL(...) ALOGD( "threadpool", __VA_ARGS__); if(true)
#define LOG_ONEWAY(...) ALOGE("ipc", __VA_ARGS__) ; if (true)
...
void IPCThreadState::joinThreadPool(bool isMain)
{
    ...
    ALOGV("**** THREAD %p (PID %d) IS JOINING THE THREAD POOL", (void*)pthread_self(), getpid());
    ...
    ALOGV("start to set_sched_policy, mMyThreadId %d", mMyThreadId);
                                  ...
                                  Parcel* reply, uint32_t flags)
{
    ...
    //printf("IPCThreadState::transact err %d\n",  err);

    if (err == NO_ERROR) {
        ALOGI(">>>> SEND from pid %d uid %d %s", getpid(), getuid(),
...
status_t IPCThreadState::talkWithDriver(bool doReceive)
{
    ...
    //ALOGD("IPCThreadState::talkWithDriver start");
    binder_write_read bwr;
    
    // Is the read buffer empty?
    const bool needRead = mIn.dataPosition() >= mIn.dataSize();
    //ALOGD("talkWithDriver  mIn.dataPosition() %d, mIn.dataSize() %d needRead %d", mIn.dataPosition(), mIn.dataSize(),needRead);
    ...
    //ALOGD("talkWithDriver  outDataSize %d,  out avail %d", mOut.dataSize(), outAvail);
...
#if 1//defined(HAVE_ANDROID_OS)
        //ALOGI("md ioctl start");
    ...
    //ALOGD("IPCThreadState::talkWithDriver err %d", err);
    if (err >= NO_ERROR) {
    ...
    //ALOGD("IPCThreadState::talkWithDriver end err %d", err);
    return err;
}
...
{
    //ALOGD("IPCThreadState::writeTransactionData start");
    alog << data;
	binder_transaction_data tr;
    ...
    if (err == NO_ERROR) {
        ...
        //ALOGD("tr data_size %d, tr.data.ptr.buffer %x, offsets_size %d data.ptr.offsets %d",
        //		data.ipcDataSize(), data.ipcData(), data.ipcObjectsCount()*sizeof(size_t), data.ipcObjects());
    ...
    //ALOGD("IPCThreadState::writeTransactionData mOut size %d", sizeof(tr));
    return NO_ERROR;
}
...
status_t IPCThreadState::executeCommand(int32_t cmd)
{
    ...
    //ALOGD("IPCThreadState::executeCommand cmd %d", cmd);
    switch (cmd) {
    case BR_ERROR:
