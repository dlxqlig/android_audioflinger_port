...
{
    printf("#####MemoryHeapBase::MemoryHeapBase\n");
}
      ...
      mDevice(0), mNeedUnmap(false), mOffset(0)
{
    ...
    printf("#####1MemoryHeapBase::MemoryHeapBase called fd %d\n", fd);
      ...
      mDevice(0), mNeedUnmap(false), mOffset(0)
{
    ...
    printf("#####2MemoryHeapBase::MemoryHeapBase called fd %d\n", fd);
    if (fd >= 0) {
        const size_t pagesize = getpagesize();
        size = ((size + pagesize-1) & ~(pagesize-1));
        if (mapfd(fd, size) == NO_ERROR) {
...
{
    const size_t pagesize = getpagesize();
    printf("#####3MemoryHeapBase::MemoryHeapBase called fd %d\n", fd);
    size = ((size + pagesize-1) & ~(pagesize-1));
