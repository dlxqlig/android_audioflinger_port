...
#define CMN_MSG ((std::string("AudioFlinger::")+ std::string(__FUNCTION__)).c_str())
// ----------------------------------------------------------------------------
...
static int load_audio_interface(const char *if_name, audio_hw_device_t **dev)
{
    ...
    ALOGD("[%s]name %s module id %d", __FUNCTION__, if_name, AUDIO_HARDWARE_MODULE_ID);
    ...
    ALOGD("start to audio_hw_device_open");
      ...
      mBtNrecIsOff(false)
{
    ...
    ALOGD("start to open audio output manager!");
    // open default alsa device
    mAudioOutputManager = new AudioOutputManager(this);
    if (mAudioOutputManager) {
    	mAudioOutputManager->init();
    }
}
                                           ...
                                           audio_output_flags_t flags)
{
    ...
    if (status == NO_ERROR && outStream != NULL) {
        ...
        if ((flags & AUDIO_OUTPUT_FLAG_DIRECT) ||
            ...
            ALOGD("DirectOutputThread start");
        	thread = new DirectOutputThread(this, output, id, *pDevices);
            ALOGV("openOutput() created direct output: ID %d thread %p", id, thread);
        } else {
            ALOGD("MixerThread start");
        	thread = new MixerThread(this, output, id, *pDevices);
            ALOGV("openOutput() created mixer output: ID %d thread %p", id, thread);
        }
        mPlaybackThreads.add(id, thread);
        ALOGD(" mPlaybackThreads.add end");
...
{
    return 1;//android_atomic_inc(&mNextUniqueId);
}
...
//int AudioFlinger::CreateAudioHardware()
//{
