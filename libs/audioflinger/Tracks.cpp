        ...
        mId(android_atomic_inc(&nextTrackId))
{
    ...
    ALOGV_IF(sharedBuffer != 0, "sharedBuffer: %p, size: %d", sharedBuffer->pointer(),
            sharedBuffer->size());
	ALOGV("AudioFlinger::ThreadBase::TrackBase::TrackBase start, shareBuffer %d", sharedBuffer != 0);
    ...
    if (client != 0) {
    	ALOGV("allocate cblk memory in client, size %d", size);
    ...
    if (mCblk != NULL) {
    	ALOGV("construct cblk memory in client, frameCount %d", frameCount);
...
AudioFlinger::PlaybackThread::Track::Track(
    ...
    mIsInvalid(false)
{
	ALOGV("AudioFlinger::PlaybackThread::Track::Track start");
	if (mCblk != NULL) {
