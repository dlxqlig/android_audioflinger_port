    ...
    if (*flags & IAudioFlinger::TRACK_FAST) {
      if (
        ...
        *flags &= ~IAudioFlinger::TRACK_FAST;
        ...
        uint32_t latencyMs = 20;//mOutput->stream->get_latency(mOutput->stream);
...
#endif
        ALOGV("start to new track, isTimed %d", isTimed);
        if (!isTimed) {
...
{
    EntryLogger entry("AudioFlinger::PlaybackThread::readOutputParameters");
    ...
    ALOGV("AudioFlinger::PlaybackThread::readOutputParameters mSampleRate %d mChannelMask %d mChannelCount %d mFormat %d mFrameSize %d mFrameCount %d",
    ...
    mNormalFrameCount = mFrameCount;//multiplier * mFrameCount;
...
{
    EntryLogger entry("AudioFlinger::PlaybackThread::getRenderPosition");
...
{
    EntryLogger entry("AudioFlinger::PlaybackThread::threadLoop_removeTracks");
...
{
    EntryLogger entry("AudioFlinger::PlaybackThread::threadLoop_write");
	// FIXME rewrite to reduce number of system calls
    mLastWriteTime = systemTime();
    mInWrite = true;
    int bytesWritten;
    //ALOGD("AudioFlinger::PlaybackThread::threadLoop_write start");
    // If an NBAIO sink is present, use it to write the normal mixer's submix
    if (mNormalSink != 0) {
    	ALOGD("AudioFlinger::PlaybackThread::threadLoop_write has normal sink");
#define mBitShift 2 // FIXME
    ...
    // otherwise use the HAL / AudioStreamOut directly
    	...
    	ALOGD("AudioFlinger::PlaybackThread::threadLoop_write mixBuffer %x, size %d", mMixBuffer, mixBufferSize);
        bytesWritten = (int)mOutput->stream->write(mOutput->stream, mMixBuffer, mixBufferSize);
#if 0
    	if (!fp) {
        	fp = fopen("/component/test.pcm", "w+");
        	if (!fp) {
        		ALOGV("cant open pcm file");
        	}
        }
        else {
        	ALOGV("AudioFlinger::PlaybackThread::threadLoop_mix write to file, mMixBuffer %x size %d",
        			mMixBuffer, mFrameCount*mFrameSize);
         	fwrite(mMixBuffer, 1, mFrameCount*mFrameSize, fp);
       	}
#endif
    }
    ...
    mNumWrites++;
    mInWrite = false;
    //ALOGD("AudioFlinger::PlaybackThread::threadLoop_write end");
}
...
bool AudioFlinger::PlaybackThread::threadLoop()
{
    ...
    EntryLogger entry("AudioFlinger::PlaybackThread::threadLoop");
    ...
    CpuStats cpuStats;
    const String8 myName(String8::format("thread %p type %d TID %d", this, mType, androidGetTid()));
    ...
    while (!exitPending())
    {
        ...
        { // scope for mLock
            ...
            ALOGV("AudioFlinger::PlaybackThread::threadLoop mActiveTracks.size() %d", mActiveTracks.size());
        ...
        ALOGV("AudioFlinger::PlaybackThread::threadLoop mMixerStatus %d", mMixerStatus);
        ...
        if (sleepTime == 0) {
            ...
            threadLoop_write();
            ALOGV("AudioFlinger::PlaybackThread::threadLoop mType %d mStandby %d", mType, mStandby);
        ...
        // mNormalSink below
{
    ...
    ALOGV("AudioStreamOutSink start");
    ...
    if (initFastMixer) {
    	ALOGV("AudioFlinger::MixerThread::MixerThread initFastMixer %d", initFastMixer);
        ...
        pid_t tid = androidGetTid();//mFastMixer->getTid(); 
    ...
    ALOGD("MixThread kUseFastMixer %d initFastMixer %d", kUseFastMixer, initFastMixer);
    switch (kUseFastMixer) {
    case FastMixer_Never:
    case FastMixer_Dynamic:
...
{
    EntryLogger entry("AudioFlinger::MixerThread::threadLoop_write");
...
{
    EntryLogger entry("AudioFlinger::MixerThread::threadLoop_mix");
...
AudioFlinger::PlaybackThread::mixer_state AudioFlinger::MixerThread::prepareTracks_l(
        Vector< sp<Track> > *tracksToRemove)
{
    EntryLogger entry("AudioFlinger::MixerThread::prepareTracks_l");
    ...
    for (size_t i=0 ; i<count ; i++) {
        ...
        ALOG("AudioFlinger::MixerThread::prepareTracks_l track index %d, isFastTrack %d.", i, track->isFastTrack());
        // process fast tracks
        if (track->isFastTrack()) {
            ...
            ALOG("AudioFlinger::MixerThread::prepareTracks_l is active %d", isActive);
            if (isActive) {
        ...
        if ((track->sharedBuffer() == 0) && !track->isStopped() && !track->isPausing() &&
            ...
            ALOG("AudioFlinger::MixerThread::prepareTracks_l mSampleRate %d, t->sampleRate() %d", mSampleRate, t->sampleRate());
        ...
        {
            ALOGV("track %d u=%08x, s=%08x [OK] on thread %p", name, cblk->user, cblk->server,
                    this);
            ...
            ALOGV("track %d u=%08x, s=%08x [NOT READY] on thread %p", name, cblk->user,
                    cblk->server, this);
...
{
	EntryLogger entry(" AudioFlinger::DirectOutputThread::prepareTracks_l");
	size_t count = mActiveTracks.size();
    mixer_state mixerStatus = MIXER_IDLE;
    ...
    // remove all the tracks that need to be...
    count = tracksToRemove->size();
    ALOGV("AudioFlinger::DirectOutputThread::prepareTracks_l tracksToRemove->size %d", count);
    if (CC_UNLIKELY(count)) {
        for (size_t i = 0 ; i < count ; i++) {
            const sp<Track>& track = tracksToRemove->itemAt(i);
            mActiveTracks.remove(track);
...
{
	EntryLogger entry(" AudioFlinger::DirectOutputThread::threadLoop_mix");
	AudioBufferProvider::Buffer buffer;
    size_t frameCount = mFrameCount;
    int8_t *curBuf = (int8_t *)mMixBuffer;
    ALOGV("AudioFlinger::DirectOutputThread::threadLoop_mix  mMixBuffer %x, frameCount %d", mMixBuffer, frameCount);
    ...
    if (!fp) {
    	...
    	ALOGV("AudioFlinger::DirectOutputThread::threadLoop_mix write to file, mMixBuffer %x size %d",
    			mMixBuffer, mFrameCount*mFrameSize);
    	fwrite(mMixBuffer, 1, mFrameCount*mFrameSize, fp);
    ...
    //os.open("/component/test1.pcm", ios::out|ios::app);
    //os<<mMixBuffer;
